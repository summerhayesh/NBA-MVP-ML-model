...
def year_input(caller):
    ...
    print(f"The available years for {caller} are from {first_yr} to {last_yr}")
    start_yr = (input("\nStart year: "))
...
def input_checker(start_yr, end_yr, first_yr, last_yr, current_yr, caller):
    ...
    while not start_yr or not end_yr:
        if not start_yr:
            start_yr = input("\nStart year picked is not a valid input"
                         f" pick a year within the range of {first_yr} to {last_yr}: ")
            start_yr, end_yr = input_checker(start_yr, end_yr, first_yr, last_yr, current_yr, caller)
        else:
    ...
    while not first_yr <= start_yr <= last_yr:
        ...
        start_yr = (input(f"Pick a start year greater than or equal to {first_yr} and less than the picked end year ({end_yr}): "))
        start_yr, end_yr = input_checker(start_yr, end_yr, first_yr, last_yr, current_yr, caller)
    ...
    while not first_yr <= end_yr <= last_yr:
        ...
        end_yr = input(f"Pick an end year greater than the picked start year ({start_yr}) and less than or equal to {last_yr}"
